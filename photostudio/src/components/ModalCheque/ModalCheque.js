    ...
    return (
        <div className='modalCheque' onClick={content}>
            ...
            <div className='modalCheque__content'>
                ...
                <p>Дата заказа: <span>{orderCustomer.o_dataOrder.slice(0, 10) + " " + orderCustomer.o_dataOrder.slice(12, 19)}</span></p>
                <p>Дата выполнения: <span>{orderCustomer.o_dateCompletion.slice(0, 10) + " " + orderCustomer.o_dateCompletion.slice(12, 19)}</span></p>
                <p>Готовность: <span>{orderCustomer.o_readiness.data[0] === 0 ? 'Нет' : 'Да'}</span></p>
                <p>Выдача заказа: <span>{orderCustomer.o_issuingOrder.data[0] === 0 ? 'Нет' : 'Да'}</span></p>
            </div>
        </div>
    );
};
